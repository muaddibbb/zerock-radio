...
SHOW_SCHEDULE = [
    ...
    {'key': 'singles',              'name': 'סינגלס',              'broadcaster': 'יובל ביטון',   'day': 1,    'time': '13:00', 'upload_time': '14:00', 'rerun_days_offset': 2,    'rerun_time': '11:00','wp_show_id': ''},
...
def _notify_whatsapp_matzad(show, podbean_url=''):
    ...
    this week's מצעד. Fires when the מצעד episode is queued (independent of any
    Podbean upload). Just announces the broadcaster + date."""
    try:
        broadcaster = (show.get('broadcaster') or '').strip() or 'ערן הר-פז'
        try:
            ...
            if s.get('id') == show_id:
                ...
                if success:
                    ...
                    if wp_post_id:
                        ...
                        # מצעד is Podbean-only (no_wp). Notify the 'מצעד שבועי' group with
                        # the broadcaster + the Podbean link, now that the upload is done.
                        _ms_pb = podbean_url or s.get('podbean_url') or ''
                        _ms_show = show
                        threading.Thread(target=_notify_whatsapp_matzad,
...
# ─── הפינה לשיפוטכם: per-artist picture + "מקום ראשון" promo image ───────────
# Pictures are stored keyed by the song LABEL (hash) so they travel with the
# song across weekly renewals (label is stable; song ids change each week).
PALASH_IMAGES_DIR = f"{RADIO_DIR}/palash_images"
os.makedirs(PALASH_IMAGES_DIR, exist_ok=True)
_PALASH_IMG_EXTS = ('.jpg', '.jpeg', '.png', '.webp')

def _palash_img_slug(label):
    return hashlib.sha1((label or '').strip().lower().encode('utf-8')).hexdigest()[:16]

def _palash_img_find(label):
    """Return the stored image path for a song label, or None."""
    if not (label or '').strip():
        return None
    slug = _palash_img_slug(label)
    for e in _PALASH_IMG_EXTS:
        p = os.path.join(PALASH_IMAGES_DIR, slug + e)
        if os.path.exists(p):
            return p
    return None

def _palash_img_delete(label):
    slug = _palash_img_slug(label)
    for e in _PALASH_IMG_EXTS:
        p = os.path.join(PALASH_IMAGES_DIR, slug + e)
        if os.path.exists(p):
            try: os.remove(p); print(f"[Palash] Deleted picture for {label!r}", flush=True)
            except Exception: pass

def _split_label(label):
    """'Artist - Song' → (artist, song). Falls back gracefully."""
    for sep in (' - ', ' – ', ' — '):
        if sep in label:
            a, s = label.split(sep, 1)
            return a.strip(), s.strip()
    return label.strip(), ''

def _upcoming_thursday_str(now=None):
    now = now or datetime.now()
    d = (3 - now.weekday()) % 7           # Thursday = weekday 3
    if d == 0:
        d = 7                             # today is Thursday → next one
    thu = now + timedelta(days=d)
    return f"{thu.day}.{thu.month}.{thu.strftime('%y')}"

_MATZAD_FONT = f"{RADIO_DIR}/static/fonts/Heebo-Black.ttf"

def _compose_first_place_image(photo_path, artist, song, date_str, out_path):
    """Render the 'מקום ראשון' promo (1250×1250): artist photo + Mitzad & ZeRock
    logos + מקום ראשון / date / artist / song. Requires Pillow (raqm for RTL)."""
    from PIL import Image, ImageDraw, ImageFont, ImageOps
    STATIC = f"{RADIO_DIR}/static"
    def _f(sz):
        ft = ImageFont.truetype(_MATZAD_FONT, sz)
        try: ft.set_variation_by_axes([900])
        except Exception: pass
        return ft
    def _cover(im, W, H):
    ...
    W = H = 1250
    base = _cover(Image.open(photo_path), W, H).convert('RGBA')
    # bottom dark gradient (legibility) + slight top darkening (logos)
    bg = Image.new('L', (1, H), 0)
    for y in range(H):
        t = max(0, (y - int(H * 0.42)) / (H * 0.58))
        bg.putpixel((0, y), int(235 * min(1, t ** 1.15)))
    dark = Image.new('RGBA', (W, H), (4, 8, 20, 255)); dark.putalpha(bg.resize((W, H)))
    base = Image.alpha_composite(base, dark)
    tg = Image.new('L', (1, H), 0)
    for y in range(H): tg.putpixel((0, y), int(150 * max(0, 1 - y / 300)))
    td = Image.new('RGBA', (W, H), (0, 0, 0, 255)); td.putalpha(tg.resize((W, H)))
    base = Image.alpha_composite(base, td)
    d = ImageDraw.Draw(base)
    try:
        mit = Image.open(os.path.join(STATIC, 'mitzad_round.png')).convert('RGBA')
        mw = 225; mit = mit.resize((mw, int(mw * mit.height / mit.width)))
        base.alpha_composite(mit, (38, 34))
    except Exception as e: print(f"[Promo] mitzad logo: {e}", flush=True)
    try:
        zr = Image.open(os.path.join(STATIC, 'zerock_t.png')).convert('RGBA')
    ...
    def _t(y, s, sz, fill):
        if not s: return
        d.text((cx, y), s, font=_f(sz), fill=fill, anchor='ma', direction='rtl',
               stroke_width=6, stroke_fill=(0, 0, 0))
    _t(818, 'מקום ראשון', 150, (255, 255, 255))
    _t(985, date_str, 92, (74, 170, 255))
    _t(1072, artist, 82, (255, 255, 255))
    _t(1162, song, 62, (215, 225, 240))
...
def _send_whatsapp_image(to, image_path, caption=''):
    ...
    try:
        ...
        r = _requests.post('http://127.0.0.1:7733/send-image',
                           json={'to': to, 'imageBase64': b64, 'caption': caption},
                           timeout=30)
        ok = r.status_code == 200
        print(f"[WA-Image] Sent to {to}: {ok} ({r.status_code})", flush=True)
        return ok
    except Exception as e:
        print(f"[WA-Image] Error: {e}", flush=True)
        return False


@app.route('/api/polls/<poll_id>/next-palash/<int:idx>/image', methods=['POST', 'GET', 'DELETE'])
def api_next_palash_image(poll_id, idx):
    """Admin: upload/serve/delete the picture for a הפינה לשיפוטכם artist.
    Keyed by the song label (so it travels with the song across renewals)."""
    polls = _load_polls()
    poll  = next((p for p in polls if p['id'] == poll_id), None)
    if not poll:
        return jsonify({'error': 'poll not found'}), 404
    np_list = poll.get('next_palash') or []
    label = np_list[idx].strip() if (0 <= idx < len(np_list)) else ''
    if not label:
        return jsonify({'error': 'שמור קודם את שם השיר בשורה זו'}), 400
    slug = _palash_img_slug(label)

    if request.method == 'GET':
        p = _palash_img_find(label)
        if not p:
            return ('', 404)
        return send_file(p)

    if request.method == 'DELETE':
        _palash_img_delete(label)
        return jsonify({'ok': True, 'has_image': False})

    f = request.files.get('image')
    if not f or not f.filename:
        return jsonify({'error': 'לא נבחר קובץ'}), 400
    ext = os.path.splitext(f.filename)[1].lower()
    if ext not in _PALASH_IMG_EXTS:
        return jsonify({'error': 'סוג קובץ לא נתמך (JPG/PNG/WEBP)'}), 400
    _palash_img_delete(label)   # clear any prior image for this label
    f.save(os.path.join(PALASH_IMAGES_DIR, slug + ext))
    print(f"[Palash] Saved picture for {label!r}", flush=True)
    return jsonify({'ok': True, 'has_image': True})
...
def api_add_show():
    ...
    # Mitzad: the 'מצעד שבועי' WhatsApp (broadcaster + Podbean link) fires AFTER the
    # Podbean upload completes (see _upload_and_mark_done), so the message can include
    # the link. Requires the מצעד to be uploaded as queue_to_broadcast.
...
def api_erev_albumim_register():
    ...
    # Refresh the WP weekly board so the newly-registered Friday shows immediately.
    # The board reads ערב של אלבומים slots directly from the bookings (see
    # _build_wp_schedule_slots) — no schedule entry is created here.
    threading.Thread(target=_sync_wp_board, daemon=True).start()
...
def poll_results_page(poll_id):
    ...
    rejected_votes   = [v for v in all_votes if v.get('status') == 'rejected']
    invalidated_votes = flagged_votes + rejected_votes
    tally = {s['id']: 0 for s in poll['songs']}
    for v in votes:
        for sid in (v.get('song_ids') or []):
            if sid in tally:
